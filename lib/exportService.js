...
export async function exportItems(companyId, projectId = null, format = 'pdf') {
  ...
  const data = items.map(item => ({
    ...
    phase: item.phaseId?.name || 'N/A',
    category: item.categoryId?.name || 'N/A',
    budgetedCost: `$${item.budgetedCost?.toLocaleString() || 0}`,
    actualCost: `$${item.actualCost?.toLocaleString() || 0}`,
    variance: `$${((item.actualCost || 0) - (item.budgetedCost || 0)).toLocaleString()}`,
  }));
  ...
  const columns = [
    ...
    { header: 'Budgeted Cost', dataKey: 'budgetedCost', key: 'budgetedCost' },
    { header: 'Actual Cost', dataKey: 'actualCost', key: 'actualCost' },
    { header: 'Variance', dataKey: 'variance', key: 'variance' },
  ];
...
export async function exportCategories(companyId, projectId = null, format = 'pdf') {
  ...
  const data = categories.map(category => ({
    name: category.name,
    project: category.projectId?.name || 'N/A',
    phase: category.phaseId?.name || 'N/A',
    budgetedAmount: `$${category.budgetedAmount?.toLocaleString() || 0}`,
    actualAmount: `$${category.actualAmount?.toLocaleString() || 0}`,
  }));
